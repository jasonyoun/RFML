...
print("Adding Interference")
perm1 = np.arange(dataI.shape[0])
np.random.shuffle(perm1)
interf = float(sys.argv[1])

dataI = dataI + dataI[perm1,:] * interf
dataQ = dataQ + dataQ[perm1,:] * interf

print("Converting")
trainData, trainLabel, testData, testLabel = DP.convertData(
    dataI, dataQ, labels, 0)


# Training Parameters
learning_rate = 0.001
training_steps = 5001  # input
batch_size = 32
display_step = 100
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps):
        ...
        if step % (display_step * 5) == 0 or step == 1:
            ...
            with open("Interference.txt", 'a') as f:
                f.write(str(step))
                f.write(" ")
                f.write(str(interf))
                f.write(" ")
